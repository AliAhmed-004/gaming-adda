...
class SudokuLevelsScreen extends StatefulWidget {
  const SudokuLevelsScreen({super.key, required this.soundEnabled});

  final bool soundEnabled;

  @override
  State<SudokuLevelsScreen> createState() =>
      _SudokuLevelsScreenState();
}

class _SudokuLevelsScreenState extends State<SudokuLevelsScreen>
  ...
  Future<void> _loadProgress({bool scrollToCurrent = false}) async {
    final unlocked = await SudokuProgress.highestUnlocked();
    if (!mounted) return;
  ...
  void _jumpToLevel(int level) {
    ...
    var offset = 0.0;
    for (final tier in SudokuLevels.tiers) {
  ...
  Widget _buildLevelScroller(int unlocked) {
    return CustomScrollView(
      controller: _scroll,
      slivers: [
        for (final (i, tier) in SudokuLevels.tiers.indexed) ...[
          SliverToBoxAdapter(
            child: _TierHeader(
              name: tier.name,
              start: tier.start,
      ...
      child: Stack(
        children: [
          _blurDisc(const Alignment(-1.2, -0.7), 180,
              SudokuBoard.frameLight.withValues(alpha: 0.14)),
          _blurDisc(const Alignment(1.3, -0.2), 220,
              SudokuBoard.glow.withValues(alpha: 0.10)),
          _blurDisc(const Alignment(-1.1, 0.9), 200,
              SudokuBoard.frameLight.withValues(alpha: 0.18)),
          _blurDisc(const Alignment(1.2, 1.1), 160,
              SudokuBoard.frameLight.withValues(alpha: 0.10)),
        ],
      ),
    );
  }
...
class _ProgressHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final fraction = cleared / SudokuLevels.maxLevel;
        ...
        children: [
            ...
            child: Stack(
              ...
              children: [
                CircularProgressIndicator(
                  ...
                  valueColor:
                      const AlwaysStoppedAnimation(SudokuBoard.glow),
                ),
                Center(
                  child: Text(
                    '$percent%',
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  '$cleared / ${SudokuLevels.maxLevel} cleared',
                  style: GoogleFonts.fredoka(
                    fontSize: 16,
                    fontWeight: FontWeight.w700,
                    color: Colors.white,
                ...
                Text(
                  unlocked >= SudokuLevels.maxLevel
                      ? 'Final boss awaits'
                      : 'Next up: Level $unlocked · ${SudokuLevels.shapeFor(unlocked).label}',
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: SudokuBoard.glow.withValues(alpha: 0.14),
              borderRadius: BorderRadius.circular(20),
              border: Border.all(
                color: SudokuBoard.glow.withValues(alpha: 0.55),
              ),
            ),
            child: Text(
              SudokuLevels.tierName(unlocked),
              style: GoogleFonts.nunito(
                fontSize: 12,
...
class _LevelTile extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final isTutorial = SudokuTutorial.isTutorialLevel(level);
