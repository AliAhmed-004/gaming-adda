...
class CheckersPlayScreen extends StatefulWidget {
  const CheckersPlayScreen({super.key});
...
class _CheckersPlayScreenState extends State<CheckersPlayScreen> {
  ...
  bool get _canInteract =>
      !_aiThinking && _game.winner == null && _game.turn == Side.dark;
  ...
  Future<void> _onSquareTap(Square square) async {
    ...
    if (_selected != null && _targets.contains(square)) {
      final move = _game.movesFrom(_selected!).firstWhere((m) => m.to == square);
      setState(() {
        _game.applyMove(move);
        _selected = null;
      });
      if (_game.winner == null && _game.turn == Side.light) {
        await _runAiTurn();
      }
      return;
    }
